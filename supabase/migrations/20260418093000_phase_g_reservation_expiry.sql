...
where status in ('draft', 'pending_deposit')
...
create or replace function public.release_expired_reservations(p_as_of timestamptz default now())
returns table (
  reservation_id uuid,
  booking_code text,
  branch_id uuid,
...
begin
  if not public.has_internal_access() then
    raise exception 'Internal access required to release expired reservations.';
  end if;
  ...
  for v_reservation in
    ...
    set status = 'expired',
        updated_at = now()
    where status in ('draft', 'pending_deposit')
      and coalesce(expires_at, created_at + interval '30 minutes') <= p_as_of
    returning *
  loop
    v_expires_at := coalesce(v_reservation.expires_at, v_reservation.created_at + interval '30 minutes');

    update public.payment_requests
    set status = case
          when status in ('sent', 'pending_verification') then 'expired'
          else status
        end,
        updated_at = now()
    where reservation_id = v_reservation.id;

    update public.reservation_room_items
    set status = 'released',
        updated_at = now()
    where reservation_id = v_reservation.id
      and status = 'active';
    ...
    update public.availability_requests
    set status = case
          when status in ('new', 'in_review', 'quoted', 'converted') then 'expired'
          else status
        end,
        closed_at = coalesce(closed_at, p_as_of),
        updated_at = now()
    where id = v_reservation.availability_request_id;
    ...
    perform public.log_audit_event(
      ...
      null,
      v_reservation.hold_id,
      v_reservation.id,
      v_reservation.availability_request_id,
      null,
      'system',
      jsonb_build_object(
        'booking_code', v_reservation.booking_code,
