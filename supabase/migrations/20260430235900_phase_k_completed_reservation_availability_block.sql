-- Completed reservations should continue to block overlapping stay windows even after the room item is released.

create or replace function public.room_has_blocking_reservation(
  p_room_id uuid,
...
as $$
  select case
    when p_stay_start_at is null or p_stay_end_at is null then false
    else exists (
      select 1
      ...
      where items.room_id = p_room_id
        ...
        and (
          (reservations.status in ('draft', 'pending_deposit', 'confirmed') and items.status = 'active')
          or (reservations.status = 'completed' and items.status in ('active', 'released'))
        )
    )
  end;
$$;
